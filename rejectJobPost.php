...
INNER JOIN tbljob ON tbljob.companyId = tblpartner.companyUsername WHERE jobId=?"))
      {
      ...
      if ($stmt = $mysqli->prepare("DELETE FROM tbljob WHERE jobId = ? LIMIT 1"))
      {
...
?>
      <script>
      alert('Job Anouncement Rejected ...')
...
if(!$mail->send()) {
  ...
  <?php
    ...
    {
   header("Location: approveJobPost.html");
